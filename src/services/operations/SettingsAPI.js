...
export function updateProfile(token, formData,navigate) {
  return async (dispatch) => {
    ...
    try {
      ...
      // const updatedUserResponse = await apiConnector("GET", GET_USER_DETAILS_API, {
      //   Authorization: `Bearer ${token}`,
      // });

      const updatedUserDetails = response.data.Details;
      const updatedUser = response.data.userDetails;
      console.log("updatedUserDetails: ", updatedUserDetails);
      // Check if updatedUserDetails is an object and has the image property
      const userImage = updatedUserDetails && updatedUser.image
        ? updatedUser.image
      ...
      window.alert("Please log out and then log in again")
